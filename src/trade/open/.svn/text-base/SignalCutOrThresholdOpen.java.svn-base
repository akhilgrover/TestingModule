 ...
 * @author Admin
 */
public class SignalCutOrThresholdOpen extends AbstractOpen
{
    public SignalCutOrThresholdOpen()
    {
        this.paramCount=2;
    }

    public SignalCutOrThresholdOpen(HashMap params,int buy)
    {
        this.buy=buy;
        this.params=params;
        this.paramCount=2;
    ...
    public TradeList fillOpen(IndicatorList indList, ShareList sl, Filters filter)
    {
        ...
        for(int i=1;i<indList.getSize();i++)
        {
            ...
            if((valid && valid1) || (valid && ((buy==1 && indf.getValue()>indf.getSignal() && indfl.getValue()<=indfl.getSignal()) ||
