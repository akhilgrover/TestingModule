 ...
 * @author Admin
 */
public class ChangeDaysCutClose extends AbstractClose
{
    /**
     * 
     */
    public ChangeDaysCutClose()
    {
        this.paramCount=2;
    }
     ...
     * @param buy
     */
    public ChangeDaysCutClose(HashMap param, int buy)
    {
        this.buy=buy;
        this.params=param;
        this.paramCount=2;
        ...
        for(int i=0;i<trdList.getSize();i++)
        {
            ...
            if(startIndex>-1)
            {
                for(int j=startIndex+1;j<indList.getSize();j++)
                {
                    ...
                    {
                        if((buy==0 && indf.getValue()>=low+pct ) || (buy==1 && indf.getValue()<=high-pct ))
                        {
                            endIndex=sl.isDatePresent(indf.getDDate());
                            if(endIndex==-1)
                                endIndex=sl.isLowerDatePresent(indf.getDDate());
                ...
                int start=sl.isDatePresent(trdOld.getStartDate());
                if(endIndex>start+days && start+days<sl.getSize())
                    endIndex=start+days;
                if(endIndex==indList.getSize()-1)
                {
                    //System.out.println(trdOld.getStartDate());
                    continue;
                }
	...
	public String toString()
	{
            ...
            buffer.append(" Change or ");
            buffer.append(days);
            buffer.append("d Close ");
            return buffer.toString();
	}

//    public int compareTo(AbstractClose o) {
//        if(!(o instanceof SimpleChangeClose) || o==null)
//            return -1;
//        else
//        {
