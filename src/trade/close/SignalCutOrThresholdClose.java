 ...
 * @author Admin
 */
public class SignalCutOrThresholdClose extends AbstractClose
{

    public SignalCutOrThresholdClose()
    {
        this.paramCount=2;
    }

    public SignalCutOrThresholdClose(HashMap param, int buy)
    {
        this.buy=buy;
        this.params=param;
        this.paramCount=2;
        ...
        for(int i=0;i<trdList.getSize();i++)
        {
            ...
            if(startIndex>-1)
            {
                for(int k=startIndex+1;k<=endIndex;k++)
                {
                    ...
                    if(valid1 || (valid && ((buy==1 && indf.getValue()<indf.getSignal() && indfl.getValue()>=indfl.getSignal()) ||
