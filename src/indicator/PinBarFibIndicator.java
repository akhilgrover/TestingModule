...
public class PinBarFibIndicator extends AbstractIndicator {
    ...
    public IndicatorList buildIndicator(ShareList sl) {
        ...
        MA avgTrend=new MA((int)param2, MA.Exponential);
        ArrayList<ShareData> nsl=new ArrayList<>();
        for (int k = 1; k < sl.getSize(); k++) {
            double val=0.0;
            ShareData sd = sl.getSharedata(k);
            nsl.add(sd);
            TreeSet<Double> fibLevels=null;
            if (k >= 30) {
                double[] mm=getMaxMin(nsl);
                fibLevels=Fibonacci.fibonacciRetracement(mm[0], mm[1]);
            }
            if(k>=30-1){
                nsl.remove(0);
            }
                ...
                if (hl >= hlavg * param3) {
                    if (ema < sd.getClosePrice()) {
                        ...
                        if (sd.getOpenPrice() <= sd.getLow() + diff) {
                            //-1
                            Double d=fibLevels.lower( sd.getLow());
                            boolean doT = true;
                            if (d != null && Math.abs((d - sd.getLow()) * 100 / d) < 0.5) {
                                doT = false;
                            } else {
                                d = fibLevels.higher(sd.getLow());
                                if (d != null && Math.abs((d - sd.getLow()) * 100 / d) < 0.5) {
                                    doT = false;
                                }
                            }
                ...
                if (hl >= hlavg * param3) {
                    if (ema > sd.getClosePrice()) {
                        ...
                        if (sd.getOpenPrice() >= sd.getHigh() - diff) {
                            //+1
                            Double d = fibLevels.lower(sd.getHigh());
                            boolean doT = true;
                            if (d != null && Math.abs((d - sd.getHigh()) * 100 / d) < 0.5) {
                                doT = false;
                            } else {
                                d = fibLevels.higher(sd.getHigh());
                                if (d != null && Math.abs((d - sd.getHigh()) * 100 / d) < 0.5) {
                                    doT = false;
                                }
                            }
